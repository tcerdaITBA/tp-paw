...
public final class ProductTestUtils {
	...
	private static Product.ProductBuilder productBuilder(int id) {
		...
		return Product.getBuilder(name, "Short Description " + id).id(id).description("Description " + id)
				.website("http://www.productseek.com/" + id).creator(dummyUser(id)).logo(name.getBytes())
