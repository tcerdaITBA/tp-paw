...
public class ShowProductController {
	...
	public ModelAndView getProduct(@PathVariable final int productId) throws ProductNotFoundException {
	...
	public ModelAndView postComment(@PathVariable final int productId,
			...
			@Valid @ModelAttribute("commentsForm") final FormComments form, final BindingResult errors,
		...
		if (errors.hasErrors()) {
			LOGGER.warn("User {} failed to post comment: form has errors: {}", loggedUser.getUserId(),
					errors.getAllErrors());
		...
		if (parentId.isPresent()) {
			comment = commentService.createComment(postedForm.getContent(), parentId.get(), productId,
					loggedUser.getUserId());
			LOGGER.info("User with id {} posted comment with id {} in reply to comment with id {}",
					loggedUser.getUserId(), comment.getId(), parentId.get());
		} else {
	...
	private void setErrorState(int productId, FormComments form, final BindingResult errors, RedirectAttributes attr, String errorForm) {
		attr.addFlashAttribute("org.springframework.validation.BindingResult.commentsForm", errors)
			.addFlashAttribute("commentsForm", form).addFlashAttribute("form", errorForm);
	}
}